...
$(document).ready(function(){
	$('#submit').click(function() {
		...
		$.ajax({
			type : 'POST',
			url : prj_broot+'index.php?section=admin&action=login',
			dataType : 'json',
			...
			success : function(data)
			{
				...
				if(typeof(data.fineMsg)!='undefined' && data.fineMsg!='')
					window.location.href=prj_broot+'index.php?section=admin&action=dashboard';
				else
					$('#message[for=login-msg]').removeClass().addClass('error').html(data.errMsg).show();
			}
		});
		return false;
	});
...
function processMsgPosted(data)
{
	...
	if(typeof(data.fineMsg) != 'undefined' && data.fineMsg != '')
	{
		...
		var eventMetaData = {};
		intercomEvent('Message Posted', eventMetaData);
	}
	else
		alert(data.errMsg);
}
...
function intercomEvent(eventName, eventMeta)
{
	//eventMeta.push({action_email: curUserEmail});
	Intercom('trackEvent', eventName, eventMeta);
}
	...
	jQuery('.formToSubmit').submit(function(e) {
		...
		jQuery.ajax({
			...
			success : function(data)
			{
				...
				if(typeof(data.intercom) != 'undefined' && data.intercom != '')
				{
					curUserEmail = data.intercom.eventData.action_email;
					intercomEvent(data.intercom.eventName, data.intercom.eventData);
				}
			}
		});
		return false;
